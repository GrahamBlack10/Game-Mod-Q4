...
const int	ARENA_POWERUP_MASK = ( 1 << Super5 ) | ( 1 << Upgrade3 ) | ( 1 << Upgrade2 ) | ( 1 << Upgrade1 );
...
// RAVEN END
		...
		GivePowerUp( owner, Super1, SEC2MS( atof( value ) ) );
	} else if ( !idStr::Icmp( statname, "regen" ) && !checkOnly ) {
		GivePowerUp( owner, Super3, SEC2MS( atof( value ) ) );
	} else if ( !idStr::Icmp( statname, "haste" ) && !checkOnly ) {
		GivePowerUp( owner, Super4, SEC2MS( atof( value ) ) );
	} else if( !idStr::Icmp( statname, "ammoregen" ) && !checkOnly ) {
		GivePowerUp( owner, Super5, -1 );
...
void idPlayer::StopPowerUpEffect( int powerup ) {
	...
	if( !( 
		(inventory.powerups & ( 1 << Super1 ) ) || 
		(inventory.powerups & ( 1 << Super3 ) ) || 
		(inventory.powerups & ( 1 << Super4 ) ) || 
		(inventory.powerups & ( 1 << Super2 ) ) 
		) )	{
...
bool idPlayer::GivePowerUp( int powerup, int time, bool team ) {
	...
	if ( gameLocal.isServer ) {
		...
		msg.WriteBits( 1, 1 );
		// team flag only needed for Super5
...
const char* idPlayer::GetArenaPowerupString ( void ) {
	if ( PowerUpActive( Upgrade1 ) ) {
		return "^isct";
	} else if ( PowerUpActive( Upgrade3 ) ) {
		return "^igrd";
	} else if ( PowerUpActive( Upgrade2 ) ) {
		return "^idbl";
	} else if ( PowerUpActive( Super5 ) ) {
		return "^irgn";
	} else {
		return "^ixxx";
	}
	...
	for ( i = 0, index = 0; i < POWERUP_MAX; i++ ) {
		...
		if ( inventory.powerupEndTime[i] > gameLocal.time || inventory.powerupEndTime[i] == -1 ) {
			...
			if ( hud ) {
				...
				if( i == Debuff4 || i == Debuff5 || i == POWERUP_CTF_ONEFLAG ) {
...
// squirrel: health regen only applies if you have positive health
		if (health > 0) {
			if (PowerUpActive(Super3) || PowerUpActive(Upgrade3)) {
			...
			if (PowerUpActive(Debuff5)) {
				int healthBoundary = inventory.maxHealth;
				int healthTic = -5;
			}
		}
// RITUAL END
	...
	// Regenerate ammo
	if( gameLocal.isServer && PowerUpActive( Super5 ) ) {
		for( int i = 0; i < MAX_WEAPONS; i++ ) {
			if( inventory.weapons & ( 1 << i ) ) {
...
void idPlayer::DropPowerups( void ) {
	...
	for ( i = 0; i < POWERUP_MAX; i++ ) {
		...
		if ( i == Super3 && gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() )
			continue;
...
void idPlayer::RespawnFlags ( void ) {
	...
	for ( i = Debuff4; i < POWERUP_CTF_ONEFLAG; i++ ) {
		if ( !(inventory.powerups & ( 1 << i )) ) {
			continue;
		}		
	...
	start = GetEyePosition();
 	...
 	if ( gameLocal.isMultiplayer && entityNumber == gameLocal.localClientNum ) {
		...
		// no aim text if player is invisible
 		if ( ( trace.fraction < 1.0f ) && ( trace.c.entityNum < MAX_CLIENTS ) && ( !((idPlayer*)gameLocal.entities[ trace.c.entityNum ])->PowerUpActive( Super2 ) ) ) {
			char* teammateHealth = "";
...
bool idPlayer::AttemptToBuyTeamPowerup( const char* itemName )
{
	...
	if ( itemNameStr == "ammo_regen" ) {
		gameLocal.mpGame.AddTeamPowerup(Super5, SEC2MS(30), team);
		UpdateTeamPowerups( true );
		return true;
	}
	else if ( itemNameStr == "health_regen" ) {
		gameLocal.mpGame.AddTeamPowerup(Super3, SEC2MS(30), team);
		UpdateTeamPowerups( true );
		return true;
	}
...
// RITUAL END
	...
	if ( gameLocal.isMultiplayer || g_testDeath.GetBool() ) {
		...
		if ( attacker->IsType( idPlayer::Type ) ) {
			...
			if ( health < -20 || killer->PowerUpActive( Super1 ) ) {
				gibDeath = true;
				gibDir = dir;
				gibsLaunched = false;
...
// RAVEN END
	...
	if ( gameLocal.isMultiplayer && gameLocal.IsFlagGameType() ) {
		...
		if ( PowerUpActive( Debuff5 ) || PowerUpActive( Debuff4 ) || PowerUpActive( POWERUP_CTF_ONEFLAG ) ) {
...
//RITUAL END
	...
	if ( oldHealth > 0 && health <= 0 ) {
		...
		//gib them here
		if ( health < -20 || ( lastKiller && lastKiller->PowerUpActive( Super1 )) )	{	
