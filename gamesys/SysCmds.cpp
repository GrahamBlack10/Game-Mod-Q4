...
// RAVEN BEGIN
	if (idStr::Icmp(name, "super1") == 0) {
		player->GivePowerUp( Super1, SEC2MS( 10.0f ) );
		player->GiveItem("weapon_rocketlauncher");
		player->GiveItem("ammo_rocketlauncher");
		return;
	}

	if ( idStr::Icmp( name, "super2" ) == 0 ) {
		player->GivePowerUp( Super2, SEC2MS( 10.0f ) );
		return;
	}

	if ( idStr::Icmp( name, "super3" ) == 0 ) {
		player->GivePowerUp( Super3, SEC2MS( 10.0f ) );
		player->GiveItem("weapon_dmg");
		player->GiveItem("ammo_dmg");
		return;
	}

	if ( idStr::Icmp( name, "super4" ) == 0 ) {
		player->GivePowerUp( Super4, SEC2MS( 10.0f ) );
		player->GiveItem("weapon_shotgun");
		player->GiveItem("ammo_shotgun");
		return;
	}

	if (idStr::Icmp(name, "super5") == 0) {
		player->GivePowerUp( Super5, SEC2MS(10.0f) );
		player->GiveItem("weapon_machinegun");
		player->GiveItem("ammo_machiegun");
		return;
	}
	
	if (idStr::Icmp(name, "upgrade1") == 0) {
		player->GivePowerUp( Upgrade1, SEC2MS(4.0f) );
		return;
	}

	if (idStr::Icmp(name, "upgrade2") == 0) {
		player->GivePowerUp( Upgrade2, SEC2MS(2.0f));
		player->GiveItem("ammo_machinegun");
		player->GiveItem("ammo_shotgun");
		player->GiveItem("ammo_rocketlauncher");
		player->GiveItem("ammo_grenadelauncher");
		player->GiveItem("ammo_lightninggun");
		player->GiveItem("ammo_nailgun");
		player->GiveItem("ammo_hyperblaster");
		player->GiveItem("ammo_dmg");
		player->GiveItem("ammo_railgun");
		return;
	}

	if (idStr::Icmp(name, "upgrade3") == 0) {
		player->GivePowerUp( Upgrade3, SEC2MS(3.0f) );
		player->GiveItem("health");
		player->GiveItem("armor");
		player->GiveItem("weapon_nailgun");
		return; 
	}
	if (idStr::Icmp(name, "upgrade4") == 0) {
		player->GivePowerUp(Upgrade4, SEC2MS(3.0f));
		return;
	}
	if (idStr::Icmp(name, "upgrade5") == 0) {
		player->GivePowerUp(Upgrade5, SEC2MS(4.0f));
		return;
	}
	if (idStr::Icmp(name, "debuff1") == 0) {
		player->GivePowerUp(Debuff1, SEC2MS(2.0f));
		return;
	}
	if (idStr::Icmp(name, "debuff2") == 0) {
		player->GivePowerUp(Debuff2, SEC2MS(4.0f));
		return;
	}
	if (idStr::Icmp(name, "debuff3") == 0) {
		player->GivePowerUp(Debuff3, SEC2MS(3.0f));
		return;
	}
	if (idStr::Icmp(name, "debuff4") == 0) {
		player->GivePowerUp(Debuff4, SEC2MS(1.0f));
		return;
	}
	if (idStr::Icmp(name, "debuff5") == 0) {
		player->GivePowerUp(Debuff5, SEC2MS(4.0f));
		return;
	}
// RAVEN END
